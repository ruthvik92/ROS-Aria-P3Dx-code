...
int main(int argc, char **argv)
{
	...
	VideoCapture cap(0);
	if(!cap.isOpened())
		printf("!!!!!!!!!!!!!!!!!!!!!lost connection with device!!!!!!!!!!!!!!!!!!!!1\n");
...
//	bc.moveRobot(10,1);
	...
	strcpy(str,"data/Odometry.txt");
	FILE *filep;
	float x1=0,y1=0;
	char comm =' ';
	
	string line;
	ifstream f("coord3.txt");
	
	while(getline(f,line))
	{
		count++;
	}
	printf("Press d to start the run\n :");
	scanf("%c",&comm);
	if(comm=='d')
	{
		sleep(3);
...
//			printf("r is pressed\n");
		//	key = waitKey(3);
		//	printf("key is %d", key);
			if(waitKey(30)==1048677)
				return bc.disconnect();
			
			double diff_time = difftime(time(0),start);
		    	int elapse_time = (int)diff_time;
